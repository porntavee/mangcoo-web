...
@Component({
  selector: 'app-d-image-event-detail',
  templateUrl: './d-image-event-detail.component.html',
  styleUrls: ['./d-image-event-detail.component.css']
})
export class DImageEventDetailComponent implements OnInit {
  code: string = '';
  model: any = {};
  modelFile: any = [];
  modelVdo: any = [];
  ...
  read() {
    this.spinner.show()
    this.serviceProviderService.post('m/imageEvent/read', { code: this.code }).subscribe(response => {
      let data: any = [];
      data = response;
  ...
  galleryFileRead() {
    this.serviceProviderService.post('m/imageEvent/galleryFile/read', { code: this.code }).subscribe(data => {
      let modef: any = {};
      modef = data;
