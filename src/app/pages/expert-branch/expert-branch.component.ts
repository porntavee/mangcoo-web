...
@Component({
  selector: 'app-expert-branch',
  templateUrl: './expert-branch.component.html',
  styleUrls: ['./expert-branch.component.css']
})
export class ExpertBranchComponent implements OnInit {
  model: any = [];
  centerList: any = [
    // {code: '', title: '', titleEN: ''},
  ...
  ngOnInit(): void {
    ...
    this.readCenter();
    this.readBanner();
    this.serviceProviderService.SendIPAddress("expertBranch " + this.center);
  ...
  read() {
    this.spinner.show()
    this.serviceProviderService.post('m/expertBranch/read', { center: this.center, category: this.categorySelected.code, currentYear: this.yearSelected.code }).subscribe(response => {
      let data: any = [];
      data = response;
      this.modelAll = data.objectData;
      this.model = this.modelAll.filter(c => c.categoryCardType != "อนุมัติบัตร" && c.categoryCardType !=  "วุฒิบัตร");
      this.modelAV = this.modelAll.filter(c => c.categoryCardType == "อนุมัติบัตร");
      this.modelVV = this.modelAll.filter(c => c.categoryCardType == "วุฒิบัตร");
      this.spinner.hide();
    }, err => {
      this.spinner.hide();
    });
  ...
  readYear() {
    this.serviceProviderService.post('m/expertBranch/currentyear/read', { category: (this.categorySelected?.code ?? ""), center: this.center }).subscribe(response => {
  ...
  updateCenter(param) {
    this.serviceProviderService.post('m/expertBranch/read', { center: param, category: this.categorySelected.code, currentYear: this.yearSelected.code }).subscribe(response => {
      let data: any = [];
      data = response;
      switch (param) {
        case '001':
  ...
  readCategory() {
    this.serviceProviderService.post('m/expertBranch/currentCategory/read', { center: this.center }).subscribe(response => {
      let data: any = [];
      data = response;
  ...
  navToDetail(code: string = '') {
    this.router.navigate(['expert-branch-detail', code]);
  }
