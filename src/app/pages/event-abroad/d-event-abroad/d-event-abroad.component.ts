...
@Component({
  selector: 'app-d-event-abroad',
  templateUrl: './d-event-abroad.component.html',
  styleUrls: ['./d-event-abroad.component.css']
})
export class DEventAbroadComponent implements OnInit {
  model: any = [];
  centerList: any = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.readYear();
    this.read();
    this.serviceProviderService.SendIPAddress("EventAbroad " + this.center);
  }
  ...
  read() {
    // this.spinner.show()
    this.serviceProviderService.post('m/eventAbroad/read', { center: this.categorySelected.code, limit: this.limit, currentYear: this.yearSelected.code }).subscribe(response => {
  ...
  updateCenter(param) {
    this.serviceProviderService.post('m/eventAbroad/read', { center: param, currentYear: this.yearSelected.code }).subscribe(response => {
      let data: any = [];
      data = response;
      switch (param) {
        case '001':
  ...
  readYear() {
    this.serviceProviderService.post('m/eventAbroad/currentyear/read', { center: this.center, category: (this.categorySelected?.code ?? "") }).subscribe(response => {
  ...
  navToDetail(code: string = '') {
    this.router.navigate(['event-abroad-detail', code]);
  }
