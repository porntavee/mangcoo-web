...
@Component({
  selector: 'app-d-news',
  templateUrl: './d-news.component.html',
  styleUrls: ['./d-news.component.css']
})
export class DNewsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.path = window.location.origin + '/' + 'news-detail';
    // window.scroll(0, 0);
    this.readCategory();
    this.readBanner();
    this.read();
  ...
  read() {
    ...
    this.serviceProviderService.post('m/news/read', { center: this.center, limit: this.limit }).subscribe(response => {
      let data: any = [];
      data = response;
