...
export class LawComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.path = window.location.origin + '/' + 'law-detail';
    window.scroll(0, 0);
    this.readCategory();
    this.readCenter();
    this.readBanner();
    this.serviceProviderService.SendIPAddress("Law " + this.center);
  ...
  read() {
    // this.spinner.show()
    this.serviceProviderService.post('m/law/read', { center: this.center, category: this.categorySelected.code, currentYear: this.yearSelected.code, limit: this.limit }).subscribe(response => {
      let data: any = [];
      data = response;
  ...
  updateCenter(param) {
    this.serviceProviderService.post('m/law/read', { center: param, category: this.categorySelected.code, currentYear: this.yearSelected.code }).subscribe(response => {
      let data: any = [];
      data = response;
      switch (param) {
        case '001':
  ...
  readCategory() {
    this.serviceProviderService.post('m/law/currentCategory/read', { center: this.center }).subscribe(response => {
      let data: any = [];
      data = response;
  ...
  readYear() {
    this.serviceProviderService.post('m/law/currentyear/read', { category: (this.categorySelected?.code ?? ""), center: this.center }).subscribe(response => {
  ...
  navToDetail(code: string = '') {
    this.router.navigate(['law-detail', code]);
  }
