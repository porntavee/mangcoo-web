...
@Component({
  selector: 'app-d-knowledge-vet-detail',
  templateUrl: './d-knowledge-vet-detail.component.html',
  styleUrls: ['./d-knowledge-vet-detail.component.css']
})
export class DKnowledgeVetDetailComponent implements OnInit {
  code: string = '';
  model: any = [];
  modelFile: any = [];
  modelVdo: any = [];
  ...
  galleryFileRead() {
    this.serviceProviderService.post('m/knowledgeVet/galleryFile/read', { code: this.code }).subscribe(data => {
      let modef: any = {};
      modef = data;
