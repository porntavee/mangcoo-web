...
@Component({
  selector: 'app-d-news-detail',
  templateUrl: './d-news-detail.component.html',
  styleUrls: ['./d-news-detail.component.css']
})
export class DNewsDetailComponent implements OnInit {
  code: string = '';
  model: any = {};
  modelFile: any = [];
  modelVdo: any = [];
  ...
  read() {
    this.spinner.show()
    this.serviceProviderService.post('m/news/read', { code: this.code }).subscribe(response => {
      let data: any = [];
      data = response;
  ...
  galleryFileRead() {
    this.serviceProviderService.post('m/news/galleryFile/read', { code: this.code }).subscribe(data => {
      let modef: any = {};
      modef = data;
  ...
  galleryRead() {
    this.serviceProviderService.post('m/news/gallery/read', { code: this.code }).subscribe(data => {
      let model: any = {};
      this.model.gallery = [];
      model = data;
  ...
  shareWithFB() {
    ...
    //   { name: 'og:url', property: 'og:url', content: 'Angular SEO Integration, Music CRUD, Angular Universal' },
    //   { name: 'og:locale', property: 'og:locale', content: 'en_US' },
    //   { name: 'og:title', property: 'og:title', content: this.model.title },
      
    //   { charset: 'UTF-8' },
    // ]);

    this.metaTagService.updateTag({ property:'og:description', content:'asdasd'},"property='og:description'");

    var url = this.serviceProviderService.urlweb + "news-detail/" + this.model.code;
