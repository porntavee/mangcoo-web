...
export class LawDetailComponent implements OnInit {
  ...
  callReadLaw() {
    this.serviceProviderService.post('m/law/read', { "code": this.code }).subscribe(response => {
  ...
  galleryFileRead() {
    this.serviceProviderService.post('m/law/galleryFile/read', { code: this.code }).subscribe(data => {
      let modef: any = {};
      modef = data;
  ...
  shareWithFB() {
    var url = this.serviceProviderService.urlweb + "law-detail/" + this.modelDetail.code;
