...
@Component({
  selector: 'app-d-knowledge-vet',
  templateUrl: './d-knowledge-vet.component.html',
  styleUrls: ['./d-knowledge-vet.component.css']
})
export class DKnowledgeVetComponent implements OnInit {
  model: any = [];
  modelInterest: any = [];
  categoryList: any = [];
  ...
  ngOnInit(): void {
    ...
    window.scroll(0,0);
    this.serviceProviderService.SendIPAddress("KnowledgeVet");
  }

  callReadCategory() {
    this.serviceProviderService.post('m/knowledgeVet/currentyear/read', {}).subscribe(response => {
  ...
  callRead() {
    this.serviceProviderService.post('m/knowledgeVet/read',  {'currentYear':this.categorySelected.code}).subscribe(response => {
      var data: any = response;
      this.model = data.objectData;
      // console.log('modelKnowledge -->', this.model);
  ...
  callReadInterest() {
    this.serviceProviderService.post('m/knowledgeVet/read',  {action:'view'}).subscribe(response => {
      var data: any = response;
